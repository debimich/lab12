...
class ValuesController < ApplicationController
  ...
  def create
    @value = Value.new(val: @value_params[:val], User: current_user,
                       Definition: Definition.find(@value_params[:definition]))

    if @value.save
      redirect_to(request.referrer, alert: t('sucaddval'))
    else
